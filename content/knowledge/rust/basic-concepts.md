...
We can break the loop or continue the current iteration using `break` and `continue` keywords:
```rust
fn main() {
    let mut x: i32 = 0;
    loop {
...
`loop` is an expression and can return value through `break` (break the current loop) or through `return` (ends the function):
```rust
fn random_func() {
    let mut counter = 0;
    ...
    let result = loop {
        ...
        if counter == 10 {
            break counter * 2; // if `return` instead of `break`, println! is unreachable
        }
    };
...
With nested loops, `break` and `continue` apply to the innermost loop. But it is possible to apply it on a different level loop using loop label:
```rust
fn main() {
    let mut count = 0;
    'counting_up: loop {
